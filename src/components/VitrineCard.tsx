...
export default function VitrineCard({ id, name, description, coverUrl, produtos, lockReason }: Props) {
  ...
  const inner = (
    ...
    >
      ...
      <span className="absolute right-4 top-4 text-3xl drop-shadow">
        {locked ? "🔒" : iconFor(name)}
      </span>
  ...
  if (locked) {
    return (
      <div className="relative block overflow-hidden rounded-2xl border border-slate-200 opacity-70 shadow-card grayscale">
        {inner}
      </div>
    );
  }
